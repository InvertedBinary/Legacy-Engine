...
public class Polygon extends PropertyEngine {
	...
	public void translateTo(float nx, float ny) {
		if (vertices.get(0).x != nx || vertices.get(0).y != ny) {
			float xd = nx - vertices.get(0).x;
			float xy = ny - vertices.get(0).y;
			for (int i = 0; i < vertices.size(); i++) {
				vertices.get(i).translate(xd, xy);
			}
			endTransform();
		}
	}
