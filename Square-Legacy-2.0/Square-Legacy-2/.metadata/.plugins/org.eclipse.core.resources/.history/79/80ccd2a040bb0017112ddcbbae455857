...
import com.IB.SL.entity.projectile.DryIce;
...
import com.IB.SL.level.Level;
	...
	public class FrostCandle extends Weapon {
		
		public FrostCandle(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public FrostCandle(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.FrostCandle;
			this.name = "Forcefrost Candle";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "10";
			this.slot = slot;
		...
		public void secondary(Projectile p, Weapon w, Mob origin) {
			...
			p = new DryIce(x, y, this.angle(), origin);
