...
import com.IB.SL.entity.projectile.Pulsefire;
import com.IB.SL.graphics.Screen;
import com.IB.SL.graphics.Sprite;
import com.IB.SL.level.Level;

	public class wand_Pulsefire extends Weapon {
		
		
		public wand_Pulsefire(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_Pulsefire(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_Pulsefire;
			this.name = "Pulsefire Wand";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "300";
			this.slot = slot;
			this.id = 12;
			this.desc = "+" + (int)ATC + "\nATC\n-----\n+" + (int)MAT + "\nMAT";
			this.FIRE_RATE = Pulsefire.FIRE_RATE;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new Pulsefire(x, y, this.angle(), origin, 1.5);
			super.attack(p, this, origin);
			p = new Pulsefire(x, y, this.angle(), origin, 2);
			super.attack(p, this, origin);
			p = new Pulsefire(x, y, this.angle(), origin, 3);
			super.attack(p, this, origin);			
