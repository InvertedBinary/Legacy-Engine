...
import com.IB.SL.level.Level;
	...
	public class RingOfMajorStamina extends EquipableItem{
		int use = 0;
		public RingOfMajorStamina(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public RingOfMajorStamina(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.ring_MajorStamina;
			this.name = "Ring of Major Stamina";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "350";
			this.slot = slot;
