...
import com.IB.SL.level.Level;
	...
	public class Bandana extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public Bandana(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Bandana(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_Bandana;
			this.name = "Bandana";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "220";
