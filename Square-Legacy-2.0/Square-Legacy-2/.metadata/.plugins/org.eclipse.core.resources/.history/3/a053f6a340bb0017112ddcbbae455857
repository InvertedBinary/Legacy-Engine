...
import com.IB.SL.level.Level;
	...
	public class bow_Cloudburst extends Weapon {
		
		public bow_Cloudburst(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public bow_Cloudburst(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.bow_Cloudburst;
			this.name = "Cloudburst Longbow";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "2930";
			this.slot = slot;
