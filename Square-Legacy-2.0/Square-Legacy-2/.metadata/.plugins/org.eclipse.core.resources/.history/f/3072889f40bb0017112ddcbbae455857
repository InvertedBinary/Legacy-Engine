...
import com.IB.SL.level.Level;
	...
	public class Charm_WIS extends EquipableItem{
		int use = 0;
		public Charm_WIS(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Charm_WIS(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Charm_WIS;
			this.name = "Charm of Wisdom";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "600";
			this.slot = slot;
		...
		public void RNGGen() {
			this.WIS = Game.getGame().getPlayer().stat_WIS * 0.15;
		}
