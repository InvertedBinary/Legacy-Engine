...
import com.IB.SL.level.Level;
	...
	public class InspiringBanner extends Weapon {
		
		public InspiringBanner(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public InspiringBanner(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.InspiringBanner;
			this.name = "Inspiring Banner";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "10";
			this.slot = slot;
			this.id = 4;
			this.desc = "+" + (int)VIT + "VIT";
		...
		public void secondary(Projectile p, Weapon w, Mob origin) {
			...
			if (pl.mobhealth < pl.maxhealth) {
				roll = Math.random() * 20;
				
				if (roll < test) pl.mobhealth++;
