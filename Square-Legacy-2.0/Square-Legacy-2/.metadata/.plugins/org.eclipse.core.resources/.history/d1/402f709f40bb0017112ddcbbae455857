...
import com.IB.SL.level.Level;
	...
	public class Charm_ATC extends EquipableItem{
		int use = 0;
		public Charm_ATC(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Charm_ATC(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Charm_ATC;
			this.name = "Charm of Attack";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "600";
			this.slot = slot;
