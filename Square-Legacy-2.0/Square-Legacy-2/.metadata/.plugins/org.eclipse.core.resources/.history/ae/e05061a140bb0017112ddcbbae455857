...
import com.IB.SL.level.Level;
	...
	public class IronSkullcap extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public IronSkullcap(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public IronSkullcap(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_IronSkullcap;
			this.name = "Iron Skullcap";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "315";
			this.slot = slot;
