...
import com.IB.SL.level.Level;
	...
	public class FrostflamePentagram extends EquipableItem{
		int use = 0;
		public FrostflamePentagram(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public FrostflamePentagram(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.FrostflamePentagram;
			this.name = "Frostflame Pentagram";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1245";
			this.slot = slot;
