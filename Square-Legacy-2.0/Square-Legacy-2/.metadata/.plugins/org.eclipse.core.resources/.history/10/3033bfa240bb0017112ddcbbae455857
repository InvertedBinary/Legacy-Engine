...
import com.IB.SL.entity.projectile.SwordStab;
...
import com.IB.SL.level.Level;
	...
	public class ShortSword extends Weapon {
		...
		public ShortSword(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public ShortSword(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Shortsword;
		...
		public void secondary(Projectile p, Weapon w, Mob origin) {
			...
			p = new SwordStab(x, y, this.angle(), origin);
