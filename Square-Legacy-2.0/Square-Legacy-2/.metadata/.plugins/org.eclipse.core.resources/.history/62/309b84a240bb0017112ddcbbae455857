...
import com.IB.SL.level.Level;
	...
	public class Sandals extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public Sandals(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Sandals(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.legs_Sandals;
			this.name = "Sandals";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "110";
