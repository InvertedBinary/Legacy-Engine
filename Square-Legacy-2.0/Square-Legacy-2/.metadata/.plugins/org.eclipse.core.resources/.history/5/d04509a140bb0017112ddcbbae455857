...
import com.IB.SL.level.Level;
	...
	public class GoldGreaves extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public GoldGreaves(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public GoldGreaves(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.legs_GoldGreaves;
