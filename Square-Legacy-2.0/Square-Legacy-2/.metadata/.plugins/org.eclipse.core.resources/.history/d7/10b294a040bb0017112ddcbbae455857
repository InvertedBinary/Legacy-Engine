...
import com.IB.SL.entity.projectile.DryFire;
...
import com.IB.SL.level.Level;
	...
	public class FireCandle extends Weapon {
		
		public FireCandle(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public FireCandle(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.FireCandle;
			this.name = "Forcefire Candle";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "10";
			this.slot = slot;
		...
		public void secondary(Projectile p, Weapon w, Mob origin) {
			...
			p = new DryFire(x, y, this.angle(), origin);
