...
public class SkeletalMage extends Mob {

	transient private AnimatedSprite down = new AnimatedSprite(SpriteSheet.SkeletalMage_down, 16, 16, 3);
	transient private AnimatedSprite up = new AnimatedSprite(SpriteSheet.SkeletalMage_up, 16, 16, 3);
	transient private AnimatedSprite left = new AnimatedSprite(SpriteSheet.SkeletalMage_left, 16, 16, 3);
	transient private AnimatedSprite right = new AnimatedSprite(SpriteSheet.SkeletalMage_right, 16, 16, 3);
	...
	transient int blinkTimer = 0;

	public SkeletalMage(int x, int y) {
		this.maxhealth = 45;
	...
	private void updateShooting() {
		...
		if ((players.size() > 0) && fireRate <= 0 && entityState != entityState.Flee) {
			...
			if (!raycast.hasCollided()) {
				mageBolt(x, y + 4, Math.random() * Math.PI);
	...
	private void updateBlink() {
		...
		if ((players.size() > 0) && blinkTimer <= 0 && entityState != entityState.Flee) {
			double px = level.getClientPlayer().getX();
			double py = level.getClientPlayer().getY();
			double sx = this.getX();
			double sy = this.getY();
