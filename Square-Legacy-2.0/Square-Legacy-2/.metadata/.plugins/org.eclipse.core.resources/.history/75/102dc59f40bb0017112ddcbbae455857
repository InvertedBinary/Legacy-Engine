...
import com.IB.SL.level.Level;
	...
	public class CottonRobe extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		Player p;
		public CottonRobe(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public CottonRobe(int slot, Player player) {
			p = player;
			RNGGen();
			basicInitialization(slot);
		}
		
		public CottonRobe(int slot) {
			p = null;
			RNGGen();
			basicInitialization(slot);
		}
		...
		public void basicInitialization(int slot) {
			this.sprite = Sprite.chest_CottonRobe;
