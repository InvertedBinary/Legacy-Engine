...
import com.IB.SL.level.Level;
	...
	public class SunAcolyteCloak extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public SunAcolyteCloak(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public SunAcolyteCloak(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.chest_SunAcolyteCloak;
			this.name = "Cloak of The Sun Acolyte";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "550";
			this.slot = slot;
