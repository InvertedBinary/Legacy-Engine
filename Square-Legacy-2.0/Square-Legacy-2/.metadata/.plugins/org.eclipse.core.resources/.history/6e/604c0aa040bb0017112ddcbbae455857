...
import com.IB.SL.level.Level;
	...
	public class DemonicTopHat extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public DemonicTopHat(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public DemonicTopHat(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_DemonicTopHat;
			this.name = "Demonic Top Hat";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1800";
			this.slot = slot;
