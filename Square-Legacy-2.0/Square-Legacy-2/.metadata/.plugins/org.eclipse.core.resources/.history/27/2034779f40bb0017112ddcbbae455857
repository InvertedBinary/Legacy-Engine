...
import com.IB.SL.level.Level;
	...
	public class Charm_EDR extends EquipableItem{
		int use = 0;
		public Charm_EDR(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Charm_EDR(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Charm_EDR;
			this.name = "Charm of Endurance";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "600";
			this.slot = slot;
		...
		public void RNGGen() {
			this.EDR = Game.getGame().getPlayer().stat_EDR * 0.15;
		}
