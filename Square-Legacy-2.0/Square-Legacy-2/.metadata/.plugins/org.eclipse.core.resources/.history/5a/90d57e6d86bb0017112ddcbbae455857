...
public class SandHaven extends Level implements Serializable{
	...
	protected void loadLevel(String path) {
		...
		try {
			BufferedImage image = ImageIO.read(SandHaven.class.getResource(path));
		...
		try {
			BufferedImage overlayImage = ImageIO.read(SandHaven.class.getResource("/overlays" + path));
		...
		Game.currentLevelId = Maps.SandHavenId;
	...
	public void checkExits(Player player, Level level, int x, int y) { 
		...
		player.setPosition(620, 441, Maps.mainId, true); 
		player.x += 8;
		}
