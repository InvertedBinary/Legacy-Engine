...
public class Archer extends Mob {
	...
	transient private AnimatedSprite down = new AnimatedSprite(SpriteSheet.Archer_down, 16, 16, 3);
	transient private AnimatedSprite up = new AnimatedSprite(SpriteSheet.Archer_up, 16, 16, 3);
	transient private AnimatedSprite left = new AnimatedSprite(SpriteSheet.Archer_left, 16, 16, 3);
	transient private AnimatedSprite right = new AnimatedSprite(SpriteSheet.Archer_right, 16, 16, 3);
	...
	transient public int trapRate = 0;
	...
	private void updateTrap() {
		...
		if ((players.size() > 0) && trapRate <= 0 ) {
			...
			raycast = level.RayCast(new Vector2i(x, y), dir, (int) 10);
			if (!raycast.hasCollided()) {
				slowTrap(x+ Math.random() * 160 - 80, y + 4 + Math.random() * 160 - 80 , dir);
				level.getClientPlayer().incombat = true;
				Sound.Play(Sound.Spell2, false);
				trapRate = 60;
			} else {
