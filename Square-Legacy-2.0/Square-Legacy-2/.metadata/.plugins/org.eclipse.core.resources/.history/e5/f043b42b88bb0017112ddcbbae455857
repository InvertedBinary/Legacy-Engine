...
public class RedShockwave extends Projectile{
	
	public static int FIRE_RATE = 30;
	public static int time = 0;
	
	public RedShockwave(double x, double y, double dir) {
		super(x, y, dir);
		range = random.nextInt(80) + 260;
		speed = 1.6;
		damage = 3;
		sprite = Sprite.RedShockwave;
	...
	protected void move() {
		...
		sprite = Sprite.rotate(Sprite.RedShockwave, Math.PI / 20.0 + random.nextInt());
