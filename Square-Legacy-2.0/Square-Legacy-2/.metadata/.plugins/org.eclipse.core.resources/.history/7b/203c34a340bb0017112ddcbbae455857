...
import com.IB.SL.level.Level;
	...
	public class Tack extends Weapon {
		
		public Tack(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Tack(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Tack;
			this.name = "Tack";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "10";
		...
		public void secondary(Projectile p, Weapon w, Mob origin) {
			...
			if (pl.stamina < pl.maxstamina) {
				roll = Math.random() * 20;
				
				if (roll < test) pl.stamina++;
