...
import com.IB.SL.level.Level;
	...
	public class wand_Trappers extends Weapon {
		
		public wand_Trappers(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_Trappers(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_Trappers;
			this.name = "Trapper's Rod";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1780";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new TrappersOrb(x, y, this.angle(), origin);
