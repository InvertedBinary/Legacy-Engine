...
import com.IB.SL.level.Level;
	...
	public class TatteredCloth extends EquipableItem{
		int use = 0;
		public TatteredCloth(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public TatteredCloth(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.TatteredCloth;
