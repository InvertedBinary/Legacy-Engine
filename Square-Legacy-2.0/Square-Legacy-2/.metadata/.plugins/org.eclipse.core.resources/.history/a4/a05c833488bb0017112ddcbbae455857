...
public class GazerBolt extends Projectile{
	
	public static int FIRE_RATE = 25;
	public static int time = 0;
	int slowed = 0;
	boolean boolSlowed = false;
	public GazerBolt(double x, double y, double dir) {
		super(x, y, dir);
		range = random.nextInt(200) + 40;
		speed = 1 + random.nextInt(4);
		damage = 3 + random.nextInt(5);
		double angleMod = random.nextDouble() * Math.PI / 2 - Math.PI / 4;
		this.angle += angleMod;
		sprite = Sprite.GazerBolt;
