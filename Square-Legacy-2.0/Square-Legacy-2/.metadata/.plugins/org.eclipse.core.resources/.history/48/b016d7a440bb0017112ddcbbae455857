...
import com.IB.SL.level.Level;
	...
	public class wand_BlastingRod extends Weapon {
		
		public wand_BlastingRod(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_BlastingRod(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_BlastingRod;
			this.name = "Blasting Rod";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "740";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new BlastingBolt(x, y, this.angle(), origin);
