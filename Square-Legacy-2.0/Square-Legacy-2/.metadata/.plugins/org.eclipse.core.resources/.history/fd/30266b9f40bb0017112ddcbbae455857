...
import com.IB.SL.level.Level;
	...
	public class Charm_AGI extends EquipableItem{
		int use = 0;
		public Charm_AGI(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Charm_AGI(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Charm_AGI;
			this.name = "Charm of Agility";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "600";
			this.slot = slot;
		...
		public void RNGGen() {
			...
			this.AGI = (int)(Game.getGame().getPlayer().stat_AGI * 0.15);
		}
