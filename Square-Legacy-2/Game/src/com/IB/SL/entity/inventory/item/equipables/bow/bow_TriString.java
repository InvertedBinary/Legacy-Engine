package com.IB.SL.entity.inventory.item.equipables.bow;
...
import com.IB.SL.entity.projectile.Arrow;
import com.IB.SL.entity.projectile.Projectile;
import com.IB.SL.graphics.Screen;
import com.IB.SL.graphics.Sprite;
import com.IB.SL.level.Level;

	public class bow_TriString extends Weapon {
		
		public bow_TriString(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public bow_TriString(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.bow_ThreeString;
			this.name = "Tri-Strung Shortbow";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1980";
			this.slot = slot;
			...
			this.FIRE_RATE = Arrow.FIRE_RATE;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new Arrow(x, y, this.angle(), origin);
			super.attack(p, this, origin);
			p = new Arrow(x, y, this.angle() + Math.PI / 18, origin);
			super.attack(p, this, origin);	
			p = new Arrow(x, y, this.angle() - Math.PI / 18, origin);
			super.attack(p, this, origin);
		}
