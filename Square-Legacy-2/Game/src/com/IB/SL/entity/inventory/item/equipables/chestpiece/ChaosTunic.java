...
import com.IB.SL.level.Level;
	...
	public class ChaosTunic extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		public ChaosTunic(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public ChaosTunic(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.chest_ChoasTunic;
			this.name = "Chaos Tunic";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "810";
			this.slot = slot;
		...
		public void RNGGen() {
			this.AGI = 25 + (random.nextInt((10 + 4) - 4) - 4);
			this.DEF = 16;
		}
