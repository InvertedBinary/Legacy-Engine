...
import com.IB.SL.level.Level;
	...
	public class WyvernSkinArmor extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		public WyvernSkinArmor(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public WyvernSkinArmor(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.chest_WyvernSkinArmor;
			this.name = "Wyvern Skin Armor";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "575";
			this.slot = slot;
		...
		public void RNGGen() {
			this.AGI = 16 + (random.nextInt((3 + 4) - 4) - 4);
			this.DEF = 8;
			this.EDR = 6;
		}
