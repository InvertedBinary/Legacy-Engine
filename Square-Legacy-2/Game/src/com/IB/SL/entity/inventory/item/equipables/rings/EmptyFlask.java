...
import com.IB.SL.level.Level;
	...
	public class EmptyFlask extends EquipableItem{
		int use = 0;
		public EmptyFlask(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public EmptyFlask(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.EmptyFlask;
