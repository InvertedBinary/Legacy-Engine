...
import com.IB.SL.level.Level;
	...
	public class PyromancerCap extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public PyromancerCap(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public PyromancerCap(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_PyromancerCap;
			this.name = "Pyromancers' Cap";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1210";
			this.slot = slot;
