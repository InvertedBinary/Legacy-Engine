package com.IB.SL.entity.inventory.item.equipables.chestpiece;
...
import com.IB.SL.level.Level;
	...
	public class StuddedLeatherTunic extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		public StuddedLeatherTunic(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public StuddedLeatherTunic(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.chest_StuddedLeatherTunic;
			this.name = "Studded Leather Tunic";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "300";
			this.slot = slot;
			...
			this.desc = "+" + (int)DEF + "DEF\n+" + (int)AGI + "AGI";
			level = Game.getGame().level2;
			this.recipe.add("Leather");
			this.recipe.add("Leather");
			this.recipe.add("Iron Ingot");
			this.recipe.add("Iron Ingot");
			this.recipe.add("Copper Ingot");
			this.recipe.add("Copper Ingot");
			this.recipe.add("Leather Tunic");
		}
		...
		public void RNGGen() {
			this.AGI = 7 + (random.nextInt((6 + 2) - 2) - 2);
			this.DEF = 5;
		}
