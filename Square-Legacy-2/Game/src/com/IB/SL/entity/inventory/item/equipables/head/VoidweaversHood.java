...
import com.IB.SL.level.Level;
	...
	public class VoidweaversHood extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public VoidweaversHood(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public VoidweaversHood(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_VoidweaversHood;
			this.name = "Voidweaver's Hood";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "0";
			this.slot = slot;
