...
import com.IB.SL.level.Level;
	...
	public class BerserkersHelm extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public BerserkersHelm(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public BerserkersHelm(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_BerserkersHelm;
			this.name = "Berserkers Helm";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1550";
			this.slot = slot;
