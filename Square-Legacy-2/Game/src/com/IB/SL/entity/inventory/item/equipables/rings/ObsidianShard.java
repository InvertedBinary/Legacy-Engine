...
import com.IB.SL.level.Level;
	...
	public class ObsidianShard extends EquipableItem{
		int use = 0;
		public ObsidianShard(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public ObsidianShard(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.ObsidianShard;
