...
import com.IB.SL.level.Level;
	...
	public class CopperHelm extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public CopperHelm(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public CopperHelm(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_BronzeHelm;
			this.name = "Copper Helm";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "680";
			this.slot = slot;
