...
import com.IB.SL.level.Level;
	...
	public class IronHelm extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public IronHelm(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public IronHelm(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_IronHelm;
			this.name = "Iron Helm";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1120";
