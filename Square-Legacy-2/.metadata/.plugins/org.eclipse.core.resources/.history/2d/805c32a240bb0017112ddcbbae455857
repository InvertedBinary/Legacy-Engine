...
public class Ranger extends Mob {
	...
	transient private AnimatedSprite down = new AnimatedSprite(SpriteSheet.Ranger_down, 16, 16, 3);
	transient private AnimatedSprite up = new AnimatedSprite(SpriteSheet.Ranger_up, 16, 16, 3);
	transient private AnimatedSprite left = new AnimatedSprite(SpriteSheet.Ranger_left, 16, 16, 3);
	transient private AnimatedSprite right = new AnimatedSprite(SpriteSheet.Ranger_right, 16, 16, 3);
	...
	private void updateTrap() {
		...
		if ((players.size() > 0) && trapRate <= 0 ) {
			...
			raycast = level.RayCast(new Vector2i(x, y), dir, (int) 10);
			if (!raycast.hasCollided()) {
				stunTrap(x+ Math.random() * 160 - 80, y + 4 + Math.random() * 160 - 80 , dir);
				level.getClientPlayer().incombat = true;
				Sound.Play(Sound.Spell2, false);
				trapRate = 100;
			} else {
