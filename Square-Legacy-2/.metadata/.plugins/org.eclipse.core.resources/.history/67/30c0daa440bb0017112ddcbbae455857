...
import com.IB.SL.entity.projectile.Contradiction;
import com.IB.SL.entity.projectile.Projectile;
import com.IB.SL.graphics.Screen;
import com.IB.SL.graphics.Sprite;
import com.IB.SL.level.Level;

	public class wand_ContradictionWand extends Weapon {
		public wand_ContradictionWand(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_ContradictionWand(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_ContradictionWand;
			this.name = "Contradictory Wand";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "625";
			this.slot = slot;
			this.id = 16;
			this.desc = "+" + (int)ATC + "\nATC\n-----\n+" + (int)MAT + "\nMAT";
			this.FIRE_RATE = Contradiction.FIRE_RATE;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new Contradiction(x, y, this.angle(), origin);
            super.attack(p, this, origin);
            p = new Contradiction(x, y, this.angle() + (Math.PI/12), origin);
            super.attack(p, this, origin);
            p = new Contradiction(x, y, this.angle() - (Math.PI/12), origin);
            super.attack(p, this, origin);
            p = new Contradiction(x, y, this.angle() + (Math.PI/24), origin);
            super.attack(p, this, origin);
            p = new Contradiction(x, y, this.angle() - (Math.PI/24), origin);
            super.attack(p, this, origin);
