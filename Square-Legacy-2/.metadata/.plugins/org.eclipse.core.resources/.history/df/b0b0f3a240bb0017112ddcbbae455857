...
import com.IB.SL.level.Level;
	...
	public class SmallStone extends EquipableItem{
		int use = 0;
		public SmallStone(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public SmallStone(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.SmallStone;
			this.name = "Small Stone";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "45";
			this.slot = slot;
