...
import com.IB.SL.level.Level;
	...
	public class MagusCap extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public MagusCap(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public MagusCap(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_MagusCap;
			this.name = "Magus' Cap";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1110";
