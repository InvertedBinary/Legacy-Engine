...
import com.IB.SL.entity.projectile.FlameMace;
...
import com.IB.SL.level.Level;
	...
	public class mace_Flame extends Weapon {
		
		private transient  AnimatedSprite down;
		
		public mace_Flame(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public void basicInitialization(int slot) {
			...
			this.FIRE_RATE = FlameMace.FIRE_RATE;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			p = new FlameMace(level.getClientPlayer().getX(), level.getClientPlayer().getY(), 0, origin);
