...
import com.IB.SL.level.Level;
	...
	public class wand_MendersStaff extends Weapon {
		
		public wand_MendersStaff(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_MendersStaff(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_MendersStaff;
			this.name = "Mender's Staff";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1320";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new MendersBolt(x, y, this.angle(), origin);
