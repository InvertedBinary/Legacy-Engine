...
import com.IB.SL.entity.inventory.effects.Regen;
	...
	public class MendersTalisman extends Weapon {
		
		public MendersTalisman(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public MendersTalisman(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.MendersTalisman;
			this.name = "Mender's Talisman";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "10";
			this.slot = slot;
