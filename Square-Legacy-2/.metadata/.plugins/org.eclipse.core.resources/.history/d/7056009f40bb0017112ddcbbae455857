...
import com.IB.SL.level.Level;
	...
	public class ArcaneSandals extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public ArcaneSandals(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public ArcaneSandals(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.legs_ArcaneSandals;
			this.name = "Arcane Sandals";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1110";
			this.slot = slot;
