...
public class SouthTownHaven extends Level implements Serializable{
	...
	public SouthTownHaven(String path) {
		super(path);
	}
	...
	protected void loadLevel(String path) {
		...
		try {
			BufferedImage image = ImageIO.read(SouthTownHaven.class.getResource(path));
		...
		try {
			BufferedImage overlayImage = ImageIO.read(SouthTownHaven.class.getResource("/overlays" + path));
		...
		Game.currentLevelId = Maps.SouthTownHavenId;
	...
	public void checkExits(Player player, Level level, int x, int y) { 
		...
		player.setPosition(238, 833, Maps.mainId, true); 
		}
