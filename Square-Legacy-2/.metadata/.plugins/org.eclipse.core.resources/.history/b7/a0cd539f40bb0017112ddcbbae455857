...
import com.IB.SL.level.Level;
	...
	public class BronzeSkullcap extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public BronzeSkullcap(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public BronzeSkullcap(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_BronzeSkullcap;
			this.name = "Bronze Skullcap";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "160";
			this.slot = slot;
