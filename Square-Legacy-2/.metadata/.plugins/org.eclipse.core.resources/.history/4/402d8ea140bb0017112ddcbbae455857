...
import com.IB.SL.level.Level;
	...
	public class MaelstromRobes extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		public MaelstromRobes(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public MaelstromRobes(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.chest_MaelstromRobes;
			this.name = "Maelstrom Robes";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "790";
			this.slot = slot;
