...
import com.IB.SL.level.Level;
	...
	public class LeatherSandals extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public LeatherSandals(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public LeatherSandals(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.legs_LeatherSandals;
			this.name = "Leather Sandals";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "350";
			this.slot = slot;
