...
import com.IB.SL.level.Level;
	...
	public class AbyssalArmor extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public AbyssalArmor(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public AbyssalArmor(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.chest_AbyssalArmor;
			this.name = "Abyssal Armor";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "875";
			this.slot = slot;
