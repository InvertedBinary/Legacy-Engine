...
import com.IB.SL.level.Level;
	...
	public class DragonsArmor extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public DragonsArmor(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public DragonsArmor(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.chest_DragonArmor;
			this.name = "Dragon's Armor";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "625";
			this.slot = slot;
