...
import com.IB.SL.level.Level;
	...
	public class RampagersHelm extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public RampagersHelm(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public RampagersHelm(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_RampagersHelm;
			this.name = "Rampagers Helm";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "750";
			this.slot = slot;
