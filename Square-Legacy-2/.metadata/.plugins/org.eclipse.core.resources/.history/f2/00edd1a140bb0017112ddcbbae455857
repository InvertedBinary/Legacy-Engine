...
public class OrbweaversBolt extends Projectile {
	...
	public OrbweaversBolt(double x, double y, double dir) {
		super(x, y, dir);
		damage = 1;
		basicInitialization();
	}
	
	public OrbweaversBolt(double x, double y, double dir, Mob mob) {
		super(x, y, dir);
		damage = 1 + (mob.stat_ATC / 7);
		basicInitialization();
	}
	
	public void basicInitialization() {
		range = random.nextInt(80) + 120;
		drop = (dropChance.nextInt((6 - 1) + 1) + 1);
		speed = 1;
		sprite = Sprite.rotate(Sprite.OrbweaversBolt, angle);	
		manaCost = 0;
