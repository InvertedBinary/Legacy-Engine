...
import com.IB.SL.level.Level;
	...
	public class ShardOfTrueIce extends EquipableItem{
		int use = 0;
		public ShardOfTrueIce(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public ShardOfTrueIce(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.ShardOfTrueIce;
			this.name = "Shard of True Ice";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "970";
			this.slot = slot;
