...
import com.IB.SL.level.Level;
	...
	public class EnchantedArrow extends Weapon {
		
		public EnchantedArrow(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public EnchantedArrow(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.EnchantedArrow;
			this.name = "Enchanted Arrow";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "10";
			this.slot = slot;
		...
		public void secondary(Projectile p, Weapon w, Mob origin) {
			...
			p = new Arrow(x, y, this.angle() + Math.random() * Math.PI / 3 - Math.PI / 6, origin);
