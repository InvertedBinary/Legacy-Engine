...
public class VoidBossRoom extends Level implements Serializable{
	...
	protected void loadLevel(String path) {
		...
		try {
			BufferedImage image = ImageIO.read(VoidBossRoom.class.getResource(path));
		...
		try {
			BufferedImage overlayImage = ImageIO.read(VoidBossRoom.class.getResource("/overlays" + path));
		...
		Game.currentLevelId = Maps.voidBossRoomId;

		
		add(new VoidBoss(31, 31));
	...
	public void checkExits(Player player, Level level, int x, int y) {
		refresh();
		player.setPosition(406, 739, Maps.mainId, true); 
		player.setPosition(new TileCoord(406, 739));
			return;
		}
}
