...
public class CaveHaven extends Level implements Serializable{
	...
	protected void loadLevel(String path) {
		...
		try {
			BufferedImage image = ImageIO.read(CaveHaven.class.getResource(path));
		...
		try {
			BufferedImage overlayImage = ImageIO.read(CaveHaven.class.getResource("/overlays" + path));
		...
		Game.currentLevelId = Maps.CaveHavenId;
	...
	public void checkExits(Player player, Level level, int x, int y) { 
		...
		player.setPosition(784, 391, Maps.mainId, true); 
		}
