...
import com.IB.SL.level.Level;
	...
	public class RangersBandana extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public RangersBandana(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public RangersBandana(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_RangersBandana;
			this.name = "Rangers Bandana";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1540";
			this.slot = slot;
