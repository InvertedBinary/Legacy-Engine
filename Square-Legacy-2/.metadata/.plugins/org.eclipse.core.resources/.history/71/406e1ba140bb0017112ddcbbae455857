...
import com.IB.SL.level.Level;
	...
	public class GreenfingersCap extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public GreenfingersCap(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public GreenfingersCap(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_GreenfingersCap;
			this.name = "Greenfingers' Cap";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1220";
			this.slot = slot;
