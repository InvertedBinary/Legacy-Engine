...
import com.IB.SL.level.Level;
	...
	public class Charm_VIT extends EquipableItem{
		int use = 0;
		public Charm_VIT(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Charm_VIT(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Charm_VIT;
			this.name = "Charm of Vitality";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "600";
			this.slot = slot;
		...
		public void RNGGen() {
			this.VIT = Game.getGame().getPlayer().stat_VIT * 0.15;
		}
