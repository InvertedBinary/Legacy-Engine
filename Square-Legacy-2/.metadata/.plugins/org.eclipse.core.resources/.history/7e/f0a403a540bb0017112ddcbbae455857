...
import com.IB.SL.entity.projectile.Stygian;
import com.IB.SL.graphics.Screen;
import com.IB.SL.graphics.Sprite;
import com.IB.SL.level.Level;

	public class wand_StygianScepter extends Weapon {
		public wand_StygianScepter(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_StygianScepter(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_StygianScepter;
			this.name = "Stygian Scepter";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "650";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new Stygian(x, y, angle(), origin, 2);
			super.attack(p, this, origin);
			p = new Stygian(x, y, angle(), origin, 1.5);
			super.attack(p, this, origin);		
		}
