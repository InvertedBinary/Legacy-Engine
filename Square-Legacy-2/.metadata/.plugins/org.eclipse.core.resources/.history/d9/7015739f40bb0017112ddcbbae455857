...
import com.IB.SL.level.Level;
	...
	public class Charm_DEF extends EquipableItem{
		int use = 0;
		public Charm_DEF(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Charm_DEF(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Charm_DEF;
			this.name = "Charm of Defense";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "600";
			this.slot = slot;
		...
		public void RNGGen() {
			this.DEF = Game.getGame().getPlayer().stat_DEF * 0.15;
		}
