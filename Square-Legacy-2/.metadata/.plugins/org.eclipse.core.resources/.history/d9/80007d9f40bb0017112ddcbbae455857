...
import com.IB.SL.level.Level;
	...
	public class Charm_MDF extends EquipableItem{
		int use = 0;
		public Charm_MDF(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Charm_MDF(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Charm_MDF;
			this.name = "Charm of Healing";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "600";
			this.slot = slot;
		...
		public void RNGGen() {
			this.MDF = Game.getGame().getPlayer().stat_MDF * 0.15;
		}
