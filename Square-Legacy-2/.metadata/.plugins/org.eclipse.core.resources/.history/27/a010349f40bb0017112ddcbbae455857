package com.IB.SL.entity.inventory.item.equipables.legs;
...
import com.IB.SL.level.Level;
	...
	public class BlackleatherBoots extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public BlackleatherBoots(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public BlackleatherBoots(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.legs_BlackleatherBoots;
			this.name = "Blackleather Boots";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1310";
			this.slot = slot;
			this.slot_Default = slot_BOOTS;
