...
import com.IB.SL.level.Level;
	...
	public class BrokenHilt extends EquipableItem{
		int use = 0;
		public BrokenHilt(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public BrokenHilt(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.BrokenHilt;
			this.name = "Broken Hilt";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "45";
			this.slot = slot;
