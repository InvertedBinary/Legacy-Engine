...
import com.IB.SL.level.Level;
	...
	public class Facewrap extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public Facewrap(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Facewrap(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_Facewrap;
			this.name = "Facewrap";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1430";
