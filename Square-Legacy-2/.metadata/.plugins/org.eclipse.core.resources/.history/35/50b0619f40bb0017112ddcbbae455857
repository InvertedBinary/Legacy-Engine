...
import com.IB.SL.level.Level;
	...
	public class CarvedArmor extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public CarvedArmor(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public CarvedArmor(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.chest_CarvedArmor;
			this.name = "Athercarved Armor";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "0";
			this.slot = slot;
