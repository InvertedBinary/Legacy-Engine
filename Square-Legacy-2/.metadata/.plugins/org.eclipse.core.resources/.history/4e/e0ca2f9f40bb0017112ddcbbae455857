...
import com.IB.SL.level.Level;
	...
	public class BlackBandana extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public BlackBandana(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public BlackBandana(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_BlackBandana;
			this.name = "Black Bandana";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "670";
			this.slot = slot;
