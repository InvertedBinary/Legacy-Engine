...
import com.IB.SL.level.Level;
	...
	public class RingOfMinorHealth extends EquipableItem{
		int use = 0;
		public RingOfMinorHealth(int x, int y, int life, int amount, int slot) {
			this.x = x;
			this.y = y;
			this.xx = x;
			this.yy = y;
		...
		public RingOfMinorHealth(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.ring_MinorHealth;
			this.name = "Ring of Minor Health";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "115";
			this.slot = slot;
