...
public class Stygian extends Projectile {
	...
	public Stygian(double x, double y, double dir) {
		super(x, y, dir);
		speed = 3;
		damage = 1;
		basicInit();
	...
	public Stygian(double x, double y, double dir, Mob mob, double speed) {
		super(x, y, dir);
	...
	void basicInit() {
		...
		sprite = Sprite.Stygian;	
		manaCost = 0;
		nx = speed * Math.cos(angle);
		ny = speed * Math.sin(angle);
		item = new Item();
		this.id = 15;
		Sound.Play(Sound.StygianWand,  false);
	}
