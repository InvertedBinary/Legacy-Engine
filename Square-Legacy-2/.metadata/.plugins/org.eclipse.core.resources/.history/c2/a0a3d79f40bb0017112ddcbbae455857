...
public class CrystallineBolt extends Projectile {
	...
	public CrystallineBolt(double x, double y, double dir) {
		super(x, y, dir);
		damage = 1;
		basicInitialization();
	}
	
	public CrystallineBolt(double x, double y, double dir, Mob mob) {
		super(x, y, dir);
		damage = 1 + (mob.stat_MAT / 10);
		basicInitialization();
	}
	
	public void basicInitialization() {
		range = random.nextInt(75) + 100;
		double deviation = random.nextDouble() * Math.PI / 3 - Math.PI / 6;
		drop = (dropChance.nextInt((6 - 1) + 1) + 1);
		speed = 3;
		sprite = Sprite.rotate(Sprite.CrystallineBolt, angle);	
		manaCost = 0;
		nx = speed * Math.cos(angle + deviation);
		ny = speed * Math.sin(angle + deviation);
		item = new Item();
		this.id = 4;
