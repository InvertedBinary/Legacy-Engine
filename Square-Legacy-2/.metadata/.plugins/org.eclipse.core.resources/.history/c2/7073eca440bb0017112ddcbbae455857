...
import com.IB.SL.level.Level;
	...
	public class wand_OrbweaversRod extends Weapon {
		
		public wand_OrbweaversRod(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_OrbweaversRod(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_OrbweaversRod;
			this.name = "Orbweaver's Rod";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1360";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new OrbweaversBolt(x, y, this.angle(), origin);
			super.attack(p, this, origin);	
			p = new OrbweaversBolt(x, y, this.angle() + Math.PI / 4, origin);
			super.attack(p, this, origin);	
			p = new OrbweaversBolt(x, y, this.angle()+ 2 * Math.PI / 4, origin);
			super.attack(p, this, origin);	
			p = new OrbweaversBolt(x, y, this.angle()+ 3 * Math.PI / 4, origin);
			super.attack(p, this, origin);	
			p = new OrbweaversBolt(x, y, this.angle()+ 4 * Math.PI / 4, origin);
			super.attack(p, this, origin);	
			p = new OrbweaversBolt(x, y, this.angle()+ 5 * Math.PI / 4, origin);
			super.attack(p, this, origin);	
			p = new OrbweaversBolt(x, y, this.angle()+ 6 * Math.PI / 4, origin);
			super.attack(p, this, origin);	
			p = new OrbweaversBolt(x, y, this.angle()+ 7 * Math.PI / 4, origin);
			super.attack(p, this, origin);		
		}
