...
import com.IB.SL.entity.projectile.DrySlow;
...
import com.IB.SL.level.Level;
	...
	public class VoodooCurse extends Weapon {
		
		public VoodooCurse(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public VoodooCurse(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.VoodooCurse;
		...
		public void secondary(Projectile p, Weapon w, Mob origin) {
			...
			p = new DrySlow(x, y, this.angle(), origin);
