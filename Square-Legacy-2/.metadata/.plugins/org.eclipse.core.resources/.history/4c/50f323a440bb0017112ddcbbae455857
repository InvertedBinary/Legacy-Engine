...
import com.IB.SL.level.Level;
	...
	public class bow_TwoString extends Weapon {
		
		public bow_TwoString(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public bow_TwoString(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.bow_TwoString;
			this.name = "Bi-Strung Shortbow";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1340";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new Arrow(x, y, this.angle() + Math.PI / 36, origin);
