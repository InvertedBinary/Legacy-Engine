...
import com.IB.SL.level.Level;
	...
	public class wand_SilversmithsRod extends Weapon {
		
		public wand_SilversmithsRod(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_SilversmithsRod(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_SilversmithsRod;
			this.name = "Silversmith's Rod";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "2000";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new SilversmithBolt(x, y, this.angle(), origin);
