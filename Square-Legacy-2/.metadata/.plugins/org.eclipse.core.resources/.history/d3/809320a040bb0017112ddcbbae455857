...
import com.IB.SL.level.Level;
	...
	public class DiamondShard extends EquipableItem{
		int use = 0;
		public DiamondShard(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public DiamondShard(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.DiamondShard;
			this.name = "Diamond Shard";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "375";
			this.slot = slot;
