...
import com.IB.SL.level.Level;
	...
	public class MagusBandana extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public MagusBandana(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public MagusBandana(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_MagusBandana;
			this.name = "Magus' Bandana";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1430";
			this.slot = slot;
