...
import com.IB.SL.level.Level;
	...
	public class Charm_MAT extends EquipableItem{
		int use = 0;
		public Charm_MAT(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Charm_MAT(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Charm_MAT;
			this.name = "Charm of Magic";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "600";
			this.slot = slot;
		...
		public void RNGGen() {
			this.MAT = Game.getGame().getPlayer().stat_MAT * 0.15;
		}
