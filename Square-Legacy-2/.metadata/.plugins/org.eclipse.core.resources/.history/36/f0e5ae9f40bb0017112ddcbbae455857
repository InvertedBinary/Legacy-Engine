...
import com.IB.SL.level.Level;
	...
	public class CopperArmor extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		public CopperArmor(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public CopperArmor(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.chest_CopperArmor;
