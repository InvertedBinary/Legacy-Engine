...
import com.IB.SL.level.Level;
	...
	public class LinenRobes extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		public LinenRobes(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public LinenRobes(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.chest_LinenRobes;
			this.name = "Linen Robes";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "250";
			this.slot = slot;
