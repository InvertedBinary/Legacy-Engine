...
import com.IB.SL.level.Level;
	...
	public class Buckler extends Weapon {
		...
		public Buckler(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public Buckler(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.Buckler;
			this.name = "Buckler";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "10";
		...
		public void secondary(Projectile p, Weapon w, Mob origin) {
			Game.getGame().getPlayer().effects.addEffect(new Resist(Game.getGame().getPlayer(), 60, 10) );
