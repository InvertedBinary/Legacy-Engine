package com.IB.SL.entity.inventory.item.equipables.head;
...
import com.IB.SL.level.Level;
	...
	public class SandblastedTurban extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public SandblastedTurban(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public SandblastedTurban(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_SandblastedTurban;
			this.name = "Sandblasted Turban";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "680";
			this.slot = slot;
