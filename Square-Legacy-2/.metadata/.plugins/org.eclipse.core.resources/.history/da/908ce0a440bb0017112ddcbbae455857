...
import com.IB.SL.level.Level;
	...
	public class wand_CrystallineScepter extends Weapon {
		
		public wand_CrystallineScepter(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_CrystallineScepter(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_Crystalline;
			this.name = "Crystalline Scepter";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "840";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new CrystallineBolt(x, y, this.angle(), origin);
