...
import com.IB.SL.level.Level;
	...
	public class IronArmor extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		public IronArmor(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public IronArmor(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.chest_IronArmor;
