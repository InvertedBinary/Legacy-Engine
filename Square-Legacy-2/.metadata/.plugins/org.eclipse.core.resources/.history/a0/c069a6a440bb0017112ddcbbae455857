...
import com.IB.SL.level.Level;
	...
	public class tome_BookofXithiax extends Weapon {
		
		private transient  AnimatedSprite down;
		
		public tome_BookofXithiax(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public tome_BookofXithiax(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.tome_BookofXithiax;
			down = new AnimatedSprite(SpriteSheet.Mace_Swoosh, 32, 32, 15);
			this.name = "Book of Xithiax";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1870";
			this.slot = slot;
