...
import com.IB.SL.level.Level;
	...
	public class bow_CobaltLong extends Weapon {
		
		public bow_CobaltLong(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public bow_CobaltLong(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.bow_CobaltLong;
			this.name = "Cobalt Longbow";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "2060";
			this.slot = slot;
