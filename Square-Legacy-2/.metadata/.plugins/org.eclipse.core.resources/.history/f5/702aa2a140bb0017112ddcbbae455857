...
import com.IB.SL.level.Level;
	...
	public class MagusHelm extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public MagusHelm(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public MagusHelm(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_MagusHelm;
			this.name = "Magus' Helm";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1430";
			this.slot = slot;
