...
import com.IB.SL.entity.projectile.SwarmBug;
import com.IB.SL.graphics.Screen;
import com.IB.SL.graphics.Sprite;
import com.IB.SL.level.Level;

	public class wand_Swarm extends Weapon {
		
		public wand_Swarm(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_Swarm(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_Swarm;
			this.name = "Staff of Ryzan-Koh";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "0";
			this.slot = slot;
			this.id = 4;
			this.desc = "+" + (int)MAT + "INT\n+" + (int)WIS + "WIS";
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new SwarmBug(x, y, this.angle() + Math.random() * Math.PI / 4 - Math.PI / 8, origin);
