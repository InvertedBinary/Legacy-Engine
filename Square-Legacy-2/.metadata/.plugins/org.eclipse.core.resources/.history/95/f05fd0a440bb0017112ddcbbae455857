...
import com.IB.SL.level.Level;
	...
	public class wand_ArcaneTwig extends Weapon {
		
		public wand_ArcaneTwig(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_ArcaneTwig(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_ArcaneTwig;
			this.name = "Arcane Twig";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "10";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new WizardProjectile(x, y, this.angle(), origin);
