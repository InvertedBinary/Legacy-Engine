...
import com.IB.SL.level.Level;
	...
	public class SilverCross extends EquipableItem{
		int use = 0;
		public SilverCross(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public SilverCross(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.SilverCross;
			this.name = "Silver Cross";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "70";
			this.slot = slot;
