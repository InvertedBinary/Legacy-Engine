...
public class SpawnHaven_Deprecated extends Level implements Serializable{
	...
	public SpawnHaven_Deprecated(String path) {
		super(path);
	}
	...
	protected void loadLevel(String path) {
		...
		try {
			BufferedImage image = ImageIO.read(SpawnHaven_Deprecated.class.getResource(path));
		...
		try {
			BufferedImage overlayImage = ImageIO.read(SpawnHaven_Deprecated.class.getResource("/overlays" + path));
		...
		add(new XML_Mob(52, 70, "/XML/Entities/TestZombie.xml"));
	}
