...
import com.IB.SL.level.Level;
	...
	public class MagusTurban extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public MagusTurban(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public MagusTurban(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_MagusTurban;
			this.name = "Magus' Turban";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "2230";
			this.slot = slot;
