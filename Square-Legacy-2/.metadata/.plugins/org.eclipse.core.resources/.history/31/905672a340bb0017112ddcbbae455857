...
import com.IB.SL.level.Level;
	...
	public class TravellersCap extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public TravellersCap(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public TravellersCap(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.head_TravellersCap;
			this.name = "Traveller's Cap";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "370";
			this.slot = slot;
