...
import com.IB.SL.level.Level;
	...
	public class AmberBug extends EquipableItem{
		int use = 0;
		public AmberBug(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public AmberBug(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.AmberBug;
			this.name = "Amber Beetle";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "880";
			this.slot = slot;
