...
import com.IB.SL.level.Level;
	...
	public class LeatherBoots extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public LeatherBoots(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public LeatherBoots(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.legs_LeatherBoots;
