...
import com.IB.SL.level.Level;
	...
	public class wand_SageRod extends Weapon {
		
		public wand_SageRod(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_SageRod(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_SageRod;
			this.name = "Sage's Rod";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "2100";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new SageBolt(x, y, this.angle(), origin);
			super.attack(p, this, origin);	
			p = new SageBolt(x, y, this.angle() - Math.PI / 12, origin);
			super.attack(p, this, origin);
			p = new SageBolt(x, y, this.angle() + Math.PI / 12, origin);
			super.attack(p, this, origin);
