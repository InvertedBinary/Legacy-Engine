...
import com.IB.SL.level.Level;
	...
	public class OpticBond extends EquipableItem{
		int use = 0;
		public OpticBond(int x, int y, int life, int amount, int slot) {
			RNGGen();
		...
		public OpticBond(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.OpticBond;
			this.name = "Optical Bond";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "30";
			this.desc = "+10 WIS\n+5 END";
