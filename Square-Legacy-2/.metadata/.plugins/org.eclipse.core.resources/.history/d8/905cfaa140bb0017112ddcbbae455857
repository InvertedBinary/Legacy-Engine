...
public class PoisonMace extends Projectile {
	...
	public PoisonMace(double x, double y, double dir) {
		super(x, y, dir);
		damage = 1;
		basicInit();
	}
	
	public PoisonMace(double x, double y, double dir, Mob mob) {
		super(x, y, dir);
...
//		List<Entity> es = level.getEntities((int)level.getClientPlayer().getX(), (int)level.getClientPlayer().getY() - 4, 16);
	...
	protected void doEffect(Entity e) {
		e.effects.addEffect(new Poison(e, 180, 30));
	}
