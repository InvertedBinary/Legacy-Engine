...
import com.IB.SL.entity.projectile.Flare;
...
import com.IB.SL.level.Level;
	...
	public class wand_FlareScepter extends Weapon {
		public wand_FlareScepter(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_FlareScepter(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_FlareScepter;
			this.name = "Flare Scepter";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "250";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new Flare(x, y, this.angle(), origin);
