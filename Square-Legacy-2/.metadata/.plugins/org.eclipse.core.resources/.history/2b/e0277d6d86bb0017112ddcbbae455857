...
public class RedTownHaven extends Level implements Serializable{
	...
	public RedTownHaven(String path) {
		super(path);
	}
	...
	protected void loadLevel(String path) {
		...
		try {
			BufferedImage image = ImageIO.read(RedTownHaven.class.getResource(path));
		...
		try {
			BufferedImage overlayImage = ImageIO.read(RedTownHaven.class.getResource("/overlays" + path));
		...
		Game.currentLevelId = Maps.RedTownHavenId;
	...
	public void checkExits(Player player, Level level, int x, int y) { 
		...
		player.setPosition(607, 683, Maps.mainId, true); 
		}
