...
import com.IB.SL.level.Level;
	...
	public class wand_RefractorStaff extends Weapon {
		
		public wand_RefractorStaff(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public wand_RefractorStaff(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.wand_RefractorsStaff;
			this.name = "Refractor's Staff";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "650";
			this.slot = slot;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			...
			p = new RefractorsBolt(x, y, this.angle(), origin);
			super.attack(p, this, origin);	
			p = new RefractorsBolt(x, y, this.angle() + Math.PI / 3, origin);
			super.attack(p, this, origin);
			p = new RefractorsBolt(x, y, this.angle() + 2 * Math.PI / 3, origin);
			super.attack(p, this, origin);
			p = new RefractorsBolt(x, y, this.angle() + 3 * Math.PI / 3, origin);
			super.attack(p, this, origin);
			p = new RefractorsBolt(x, y, this.angle() + 4 * Math.PI / 3, origin);
			super.attack(p, this, origin);
			p = new RefractorsBolt(x, y, this.angle() + 5 * Math.PI / 3, origin);
			super.attack(p, this, origin);
		}
