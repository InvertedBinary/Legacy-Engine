...
import com.IB.SL.level.Level;
	...
	public class LeatherTunic extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		public LeatherTunic(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public LeatherTunic(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.chest_LeatherTunic;
			this.name = "Leather Tunic";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "290";
			this.slot = slot;
		...
		public void RNGGen() {
			this.AGI = 4 + (random.nextInt((5 + 3) - 3) - 3);
			this.DEF = 3;
		}
