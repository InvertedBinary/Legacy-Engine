...
public class FairyVanguard extends Mob{
	
	transient private AnimatedSprite anim = new AnimatedSprite(SpriteSheet.FairyVanguard_anim, 16, 16, 2);
	...
	public FairyVanguard(int x, int y) {
		this.maxhealth = 50;
    ...
    private void updateShooting() {
        ...
        if ((players.size() > 0) && fireRate <= 0) {
            ...
            fairyBolt(x, y + 4, dir);
            fairyFireBolt(x, y + 4, dir + Math.PI / 4);
            fairyStunBolt(x, y + 4, dir - Math.PI / 4);
            fairyFireBolt(x, y + 4, dir + Math.PI / 8);
            fairyStunBolt(x, y + 4, dir - Math.PI / 8);
    ...
    private void updateHeal() {
    	Mob m = (Mob)level.getEntities(this, 160).get(0);
        if ((m != null) && blinkRate <= 0) {
            double px = m.getX();
            double py = m.getY();
            double sx = this.getX();
            double sy = this.getY();
    ...
    private void updateRestore() {
        ...
        if ((players.size() > 0) && blastRate <= 0) {
            ...
            double dy = py - sy;
            double dir = Math.atan2(dy, dx);
            fairyHeal(x + Math.random() * 80 - 40, y + Math.random() * 80 - 40, Math.random() * Math.PI);
            fairyHeal(x + Math.random() * 80 - 40, y + Math.random() * 80 - 40, Math.random() * Math.PI);
            fairyHeal(x + Math.random() * 80 - 40, y + Math.random() * 80 - 40, Math.random() * Math.PI);
            fairyHeal(x + Math.random() * 80 - 40, y + Math.random() * 80 - 40, Math.random() * Math.PI);
            fairyHeal(x + Math.random() * 80 - 40, y + Math.random() * 80 - 40, Math.random() * Math.PI);
