...
import com.IB.SL.level.Level;
	...
	public class RingOfMajorMana extends EquipableItem{
		int use = 0;
		public RingOfMajorMana(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public RingOfMajorMana(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.ring_MajorMana;
			this.name = "Ring of Major Mana";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "350";
			this.slot = slot;
