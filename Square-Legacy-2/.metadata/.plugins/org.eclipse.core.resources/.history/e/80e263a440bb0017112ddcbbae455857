...
import com.IB.SL.entity.projectile.HolyMace;
...
import com.IB.SL.level.Level;
	...
	public class mace_Holy extends Weapon {
		
		private transient  AnimatedSprite down;
		
		public mace_Holy(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public mace_Holy(int slot) {
			RNGGen();
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			this.sprite = Sprite.mace_Holy;
			...
			this.FIRE_RATE = HolyMace.FIRE_RATE;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			p = new HolyMace(level.getClientPlayer().getX(), level.getClientPlayer().getY(), 0, origin);
