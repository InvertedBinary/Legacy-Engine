...
import com.IB.SL.entity.projectile.FrostMace;
...
import com.IB.SL.level.Level;
	...
	public class mace_Frostbitten extends Weapon {
		
		private transient  AnimatedSprite down;
		
		public mace_Frostbitten(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public void basicInitialization(int slot) {
			...
			this.FIRE_RATE = FrostMace.FIRE_RATE;
		...
		public void attack(Projectile p, Weapon w, Mob origin) {
			p = new FrostMace(level.getClientPlayer().getX(), level.getClientPlayer().getY(), 0, origin);
