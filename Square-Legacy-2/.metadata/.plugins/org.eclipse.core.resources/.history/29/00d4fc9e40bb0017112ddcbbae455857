...
import com.IB.SL.level.Level;
	...
	public class ArcaneFootwraps extends EquipableItem{
		int use = 0;
		boolean equipped = false;
		
		public ArcaneFootwraps(int x, int y, int life, int amount, int slot) {
			RNGGen();
			this.x = x;
			this.y = y;
			this.xx = x;
		...
		public ArcaneFootwraps(int slot) {
			basicInitialization(slot);
		}
		
		public void basicInitialization(int slot) {
			RNGGen();
			this.sprite = Sprite.legs_ArcaneFootwraps;
			this.name = "Arcane Footwraps";
			this.dropchance = -1;
			this.rarity = 0;
			this.price = "1340";
			this.slot = slot;
